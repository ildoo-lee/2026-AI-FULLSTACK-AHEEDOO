...
-- >>> (2) join  on
select   a.no , a.name , a.age , b.ban
from     join_userinfo a  join join_userban b  -- , 대신 join
on    a.no=b.no;                               -- where 대신 on
...
-- --
SELECT * FROM EMP, DEPT ORDER BY EMPNO; 
-- join 조건이 없으면 두 테이블 모두를 조회해서 경우의 수가 다 나와
...
select   *  -- a.empno , a.ename  , a.job , a.mgr  , a.hiredate , a.sal  , a.comm , a.deptno , b.deptno , b.dname , b.loc 
from     emp , dept
where    emp.deptno=dept.deptno
order by empno;

select   empno , ename  , job , mgr  , hiredate , sal  , comm , emp.deptno , dept.deptno , dept.dname , dept.loc 
from     emp  , dept 
where    emp.deptno=dept.deptno
order by empno;
-- 겹치는 필드 소속채우기
...
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO;
-- 0	67	11:24:24	SELECT EMPNO, ENAME, DEPTNO, DNAME, LOC
-- FROM EMP E, DEPT D
-- WHERE E.DEPTNO = D.DEPTNO
-- LIMIT 0, 1000	Error Code: 1052. Column 'DEPTNO' in field list is ambiguous	0.000 sec
...
-- select     모든필드 *
-- from       emp 테이블의 별칭을 E로 ,SALGRADE  테이블의 별칭을 S
-- where      사원의 급여가(sal)   between   SALGRADE의 최소급여   and  최대급여 사이이 있는
-- order      by  grade, sal 순으로
RENAME TABLE salagrade TO salgrade;
...
select  *
from    emp e natural join  dept d;  -- deptno 필드 겹침 부분이 1번만 나옴 (중간 테스트)
...
select   DEPTNO , e.deptno , DNAME  ,EMPNO , ENAME , SAL 
from     emp e natural join dept d
-- on       e.deptno=d.deptno
where    sal > 2000;
-- order by e.deptno asc;
...
select   e.DEPTNO , DNAME  , avg(sal)`AVG_SAL` , max(sal)`MAX_SAL`, min(sal)`MIN_SAL`, count(sal)`CNT` 
from     emp e, dept d
where    e.deptno=d.deptno
