...
-- 10:42:17	0	19	10:42:17	select ban, count(*) from  group_userinfo LIMIT 0, 1000	Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec
select ban, count(*) -- 6 (집계안된 일반) , 1(집계)
from  group_userinfo;

select   ban, count(*)
from     group_userinfo
-- where
group by ban;  -- 반별 (집계필드(count(*)) 빼고 다)
-- having
-- order by
-- limit
...
select  count(*)'데이터갯수' , count(empno), count(mgr)
from    emp;   -- 집계합수는 null 빼고 계산
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 연습문제-5
...
from     emp
group by deptno
having   deptno =30;   -- 집계 후 필터링

select   count(*) '부서30 직원수'
from     emp
where    deptno = 30;  -- 집계 전 필터링
...
-- 3 rows in set (0.00 sec)
--  
select   deptno, avg(sal) '부서30 중복제거 평균급여'
from     emp
group by deptno
order by deptno asc;
...
GROUP BY   DEPTNO, JOB
having     AVG(SAL) >= 2000  -- group by 필터링은 having으로
ORDER BY   DEPTNO, JOB;
