...
delete from userinfo_selecct where no>=4;
...
select  deptno , job       , count(*)`사원수`,   , max(sal)   , sum(sal),   avg(sal)`평균급여`
from emp
group by deptno , job
order by deptno , job;
...
select   12.345 
       , round(12.355, 1)   -- 소수점 1번째 자리까지 12.3 
       , round(15.345 , -1)  -- 정수 1번째 자리에서 반올림 
from dual;
...
select  123.4578, ceil(123.4578) `CEIL` 
...
select instr('abc' , 'b')`a(1)b(2)c(3)에서 b`
...
select substr('abcde' , 2,2) , substr('abcde' , 2,3) , substr('abcde' , 1,3) -- 두번째부터 2개
from dual;
-- +------+------+------+
-- | s1   | s2   | s3   |
-- +------+------+------+
...
select replace ('hello sally' , 'sally' , 'a') -- sally를 a 로 바꿔줘
from dual;
-- +--------------------------------------+
...
-- mysql> -- 10. 빈칸채우기 
select lpad('ABC' , 5 , '#') , rpad('ABC' , 5 , '#') -- 빈카을 ##으로 매꾸며 시작하여 5칸을 만들어
from dual;
-- +----------------------+----------------------+
...
insert into fn_select_userinfo (name, age, email) values ('aaa' , 11 ,'aaa@gmail.com');
insert into fn_select_userinfo (name, age, email) values ('bbb' , 22 ,'bbb@gmail.com');
insert into fn_select_userinfo (name, age, email) values ('ccc' , 33 ,'ccc@gmail.com');
insert into fn_select_userinfo (name, age, email) values ('ddd' , 44 ,'ddd@gmail.com');
insert into fn_select_userinfo (name, age, email) values ('abc' , 55 ,'abc@gmail.com');
insert into fn_select_userinfo (name, age, email) values ('bca' , 66 ,'bca@gmail.com');
...
select name, left(name , 1)`첫번째 글짜` , right(name , 1)`마지막 글짜`
from    fn_select_userinfo;
-- +------+-------------+------------+
-- | name | 첫번째 글자 | 마지막글자 |
-- +------+-------------+------------+
...
select name, replace (name , 'aaa' , 'aaa 1등')   -- 문자열에서 찾아서 바꾸기
from    fn_select_userinfo;
-- +------+-----------------------------------+
