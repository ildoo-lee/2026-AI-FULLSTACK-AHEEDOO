...
-[] 002. VIEW (HTML + CSS +JS)
    ...
    - 다음파일들에서 위에 header.jsp / footer.jsp include하기
...
C 글쓰기 
   insert into mvcboard1( bame, bpass, btitle, bcontent, bip) values(?,?,?,?,?)
...
004. CONTROLLER
       ...
       -2) write.jsp (글쓰기 폼)    write_action.jsp(글쓰기 처리)
            insert into mvcboard1( bame, bpass, btitle, bcontent, bip) 
            values(?,?,?,?,?)
