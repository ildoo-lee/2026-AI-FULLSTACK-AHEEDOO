...
정의 : 왜부에서 접근 제어
구현 : balance를 private 으로 선언. deposit, withdraw, getBalance 메서드를 통해서만 접근
...
의미 : static 공통 또는 공유
메모리 : method area 저장 , 객체생성(new)와 관계없이 접근, 클래스명.변수명, 클래스명.메서드명
적용 : Account.accountCount

Q4. final 키워드의 의미를 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.
의미 : 변경불가  클래스에서는 (상속 X)  , 변수(상수: 수정안됨), 메서드(오버라이드 X) 
id , 한번 초기화되면 변경불가, this.id = ++accountCount; 초기화 
...
static 메서드 - 객체생성없이 호출가능, 클래스에 속함
...
변수 : 값변경불가(상수), 메서드: 오버라이드 불가, 클래스:상속불가


Q10. 캡슐화의 장점은 무엇인가?
데이터보호(외부에서 직접 접근하는 것을 차단한다)
유지보수가 용이함.(내부에서 바뀌는것이 없어서) 내부 구현변경시 외부 영향 최소화



//클래스는 부품객체 위 부품들 보고 있어.

```
class Account {
    private int balance; //인스턴스변수 -개인 각각 this     heap area   new    생성자        
    public static int accountCount=0; //클래스변수 - 공유 method area   new X   
    public final int id; //인스턴스변수 - 개인 각각 this     heap area   new    생성자             

	public Account(){this.id = ++accountCount;} // 초기화, 하나 증가하고 대입하세요
    public Account(int id, int balance) {
        this(); // Account()
        this.balance = balance; 
    }
...
public class Repeat002_oop {
    public static void main(String[] args) {
        Account a1 = new Account(1, 100); // 여기서 하나 호출
        Account a2 = new Account(2, 200); // 여기서 하나 또 호출
