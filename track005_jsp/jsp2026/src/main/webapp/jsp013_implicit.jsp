   ...
   <div class="container card my-5">
      <h3 class="card-header">001 내장객체</h3>
       ...
       <h4>002. request - getParameter</h4>
		...
		<form action="jsp013_1.jsp" method="post" onsubmit="return check()">
			<div class="my-2">
		...
		<h4>003. response</h4>
        <pre class="alert alert-info">
		1. http요청에 대한 응답제장하는 객체
		2. jsp에서 다른페이지로 이동
		   1) sendRedirect - 브라우저 url을 변경 o, request/ response객체가 유지안됨
		   2) RequestDispatcher - 위임(요청전달)
		                     브라우저 url을 변경 x, request/ response객체가 유지 됨
		3. 
		4. 
		</pre>
       	<%
       	...
       	<form action="jsp013_4.jsp" method="post" onsubmit="return check()">
			<div class="my-2">
				<label for="userage">나이입력</label> <input type="text"
        ...
        <h4>005. error</h4>
       	...
       	error
       	1. 4XX : 클라이언트 오류
       	     404 (페이지 없음) / 400 (bad quest 요청 없음)
       	2. 5XX : 서버 오류
       	     500 (내부서버오류) / 502 (서버 과부하)
       	     
       	* 해결방안
       	1. error 페이지 만들기
       	2. [src] - [main] - [wepapp] - [WEB-INF] - web.xml 에러처리 설정
