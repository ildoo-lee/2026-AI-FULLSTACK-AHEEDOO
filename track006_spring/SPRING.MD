...
insert : insert into userinfo (no , email, age ) value (?,?,?); 
select (전체): select * from userinfo;
select (해당번호의 읽기): select * form userinfo where no=1; 
update (해당번호 수정) :  update userinfo set email = 'aaa@email.com', age = 30 where no = 1; 
delete (해당번호 삭제) :  delete from userinfo where no = 1;
...
[실습]
1) view.zip 다운로드
2) servlet-context.xml    확인/view/ 폴더 안에 압출풀기
